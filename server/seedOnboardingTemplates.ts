...
const TEMPLATES: TemplateSeed[] = [
  // ── 1. New listing from scratch (8-stage flow) ────────────────────────
  {
    ...
    kickoffFieldSchema: [
      ...
      { key: "address", label: "Property address", type: "text", placeholder: "123 Main St, City, ST 12345" },
      { key: "photos_link", label: "Photos link", type: "url", placeholder: "Drive / Dropbox URL" },
      { key: "use_main_leisr_account", label: "List in account", type: "text", placeholder: "Main account" },
      { key: "cleaning_fee", label: "Cleaning fee", type: "money" },
      { key: "pets_allowed", label: "Pets allowed?", type: "boolean" },
      { key: "pet_fee", label: "Pet fee", type: "money" },
      { key: "onboarding_form_complete", label: "Onboarding form completed?", type: "boolean" },
  ...
  // ── 2. Owner keeping their existing listing ──────────────────────────
  {
    ...
    kickoffFieldSchema: [
      ...
      { key: "address", label: "Property address", type: "text", placeholder: "123 Main St, City, ST 12345" },
      { key: "photos_link", label: "Photos link", type: "url", placeholder: "Drive / Dropbox URL" },
      { key: "existing_listing_url", label: "Existing listing link", type: "url", placeholder: "Airbnb / Vrbo Link" },
      { key: "airbnb_email", label: "Airbnb account email", type: "text", placeholder: "owner@email.com" },
      { key: "airbnb_password", label: "Airbnb account password", type: "text", placeholder: "••••••••" },
      { key: "cleaning_fee", label: "Cleaning fee", type: "money" },
      { key: "pets_allowed", label: "Pets allowed?", type: "boolean" },
      { key: "pet_fee", label: "Pet fee", type: "money" },
      { key: "onboarding_form_complete", label: "Onboarding form completed?", type: "boolean" },
    ...
    stagesConfig: [
      {
        ...
        ownerRole: "ethan",
        defaultChecklist: [
          { id: "ex0_owner_form_sent", label: "Send the \"Property Owner\" form to the owner" },
          { id: "ex0_local_recs_sent", label: "Send the \"Local Recs\" form to the owner" },
        ],
        defaultFields: [],
      },
      {
        ...
        label: "Intake & Documentation",
        defaultChecklist: [
          { id: "ex1_credentials_requested", label: "Request the owner's Airbnb and Vrbo login credentials" },
          { id: "ex1_dates_blocked", label: "Block all future dates (stays blocked until Stage 7)" },
          { id: "ex1_internal_doc", label: "Once owner form returned: finish the internal doc in Notion" },
          { id: "ex1_breezeway_tasks", label: "Once owner form returned: add routine maintenance tasks to Breezeway" },
          { id: "ex1_guidebook_notion", label: "Once local recs form returned: draft the guest-facing guidebook in Notion" },
        ],
        defaultFields: [],
      },
      {
        key: "account_access_audit",
        label: "Account Access & Audit",
        defaultChecklist: [
          { id: "ex2_bank_taxpayer_info", label: "Add bank and taxpayer info to the owner's Airbnb account" },
          { id: "ex2_payout_routing", label: "Set up payout routing on Airbnb — route to correct bank by state (NC → 4541, VA → 0856, SC → 0855)" },
          { id: "ex2_cohost_invite", label: "Invite Leisr Stays as co-host on Airbnb (admin@leisrstays.com, grant \"Full Access\")" },
          { id: "ex2_ratings_doc", label: "Document the owner's current Airbnb ratings for baseline comparison" },
          { id: "ex2_listing_categories", label: "Audit: fill out all listing categories" },
          { id: "ex2_backup_key", label: "Audit: confirm the backup key (ping Ethan or Chloe if none available)" },
          { id: "ex2_amenities", label: "Audit: add amenities" },
          { id: "ex2_cleaning_fee", label: "Audit: set the cleaning fee" },
          { id: "ex2_pet_fee", label: "Audit: set the pet fee (if pet-friendly)" },
          { id: "ex2_good_track_record", label: "Audit: under booking settings, select \"Good Track Record\"" },
          { id: "ex2_cancellation_policy", label: "Audit: set cancellation policy — 1–2 BR = moderate, 3–4 BR = limited, 4+ BR = firm" },
          { id: "ex2_checkin_checkout_times", label: "Audit: set check-in to 4pm and check-out to 10am" },
        ],
        defaultFields: [
          { key: "hostaway_listing_id", label: "Hostaway listing ID", type: "text" },
          { key: "baseline_rating", label: "Baseline Airbnb rating", type: "text" },
        ],
      },
      {
        key: "channel_distribution",
        label: "Channel Distribution",
        defaultChecklist: [
          { id: "ex3_airbnb_hostaway", label: "Connect Airbnb account to Hostaway and set booking window to 6 months" },
          { id: "ex3_vrbo_hostaway", label: "Connect Vrbo account to Hostaway (or migrate onto our Vrbo)" },
          { id: "ex3_vrbo_pet_fee", label: "Add the pet fee to the VRBO column in Hostaway" },
          { id: "ex3_google_export", label: "Export the listing to Google from Hostaway" },
          { id: "ex3_direct_booking_category", label: "Confirm correct category on the LS direct booking site" },
          { id: "ex3_hostaway_import_sheet", label: "Fill out the Hostaway import sheet" },
          { id: "ex3_email_tyler", label: "Email Tyler Grachan (accountant) to notify him of the new listing" },
          { id: "ex3_maintenance_spreadsheet", label: "Add property to the maintenance tasks spreadsheet" },
          { id: "ex3_stripe_chloe", label: "Slack Chloe to connect listing to Stripe (NC h9bE, SC fWzH, VA al4F)" },
          { id: "ex3_past_financials", label: "Gather the owner's past financial data" },
        ],
        defaultFields: [],
      },
      {
        ...
        label: "Automation, Fees & Policies",
        defaultChecklist: [
          { id: "ex4_early_late_fees", label: "Add early check-in and late check-out fees" },
          { id: "ex4_checkin_instructions", label: "Write custom check-in and check-out instructions" },
          { id: "ex4_guidebook_airbnb", label: "Create the guidebook in the Airbnb platform" },
          { id: "ex4_automate_messages", label: "Automate messaging: booking confirmations, check-in/out instructions, guidebook (non-Airbnb), trash day reminders" },
          { id: "ex4_fireplace_chloe", label: "If wood-burning fireplace or firepit: ping Chloe to coordinate wood delivery and firewood payment sign" },
          { id: "ex4_upcharge_fees", label: "Set up charge fees in Listing Mapping: 18% Airbnb, 5% Vrbo" },
          { id: "ex4_rental_agreement", label: "Enable the rental agreement" },
          { id: "ex4_backup_key_list", label: "If no backup key: add property to the \"no backup key\" list" },
        ],
        defaultFields: [],
      },
      {
        ...
        label: "Pricing, HostBuddy & Optimization",
        defaultChecklist: [
          { id: "ex5_listing_optimized", label: "Optimize the Airbnb listing and reimport to Hostaway" },
          { id: "ex5_pricelabs", label: "Connect to PriceLabs and configure pricing strategy" },
          { id: "ex5_los_discounts", label: "Set up LOS discounts in Hostaway" },
          { id: "ex5_zapier_workflow", label: "Add listing to the Zapier-to-Slack workflow" },
          { id: "ex5_breezeway_contact", label: "Select default maintenance contact in Breezeway" },
          { id: "ex5_hostbuddy", label: "Set up the listing in HostBuddy" },
          { id: "ex5_booking_com", label: "List on Booking.com and apply 18% markup in Hostaway" },
          { id: "ex5_early_bird", label: "Set up Early Bird discounts" },
          { id: "ex5_hostaway_review", label: "Final Hostaway settings review" },
        ],
        defaultFields: [
          { key: "pricelabs_url", label: "PriceLabs URL", type: "url" },
          { key: "hostbuddy_link", label: "HostBuddy link", type: "url" },
      ...
      },
      {
        ...
        label: "Physical Property Prep",
        defaultChecklist: [
          { id: "ex6_photos_scheduled", label: "Schedule photos (appliances, kitchen items, hot tub, etc.)" },
          { id: "ex6_fridge_one_pager", label: "Slack Chloe to create the Canva fridge one-pager" },
          { id: "ex6_code_compliance", label: "Complete code compliance check (fire alarms, etc.)" },
          { id: "ex6_turnoverbnb_breezeway", label: "Add property to TurnoverBnB and Breezeway" },
          { id: "ex6_deep_clean", label: "Coordinate initial onboarding, deep clean, and setup with cleaners" },
          { id: "ex6_cleaner_onboarded", label: "Onboard the cleaner to our processes" },
          { id: "ex6_handyman", label: "Line up a handyman and electrician" },
          { id: "ex6_landscaping", label: "Line up a landscaping company" },
        ],
        defaultFields: [],
      },
      {
        ...
        ownerRole: "ethan",
        defaultChecklist: [
          { id: "ex7_taxpayer_info", label: "Enter taxpayer info" },
          { id: "ex7_rental_agreement_review", label: "Review the rental agreement" },
          { id: "ex7_quo_owners_line", label: "Add the owner to the Quo Owners Line" },
          { id: "ex7_hostaway_user", label: "Add the owner as a user in Hostaway (limited to their property only)" },
          { id: "ex7_insurance", label: "Get added to the owner's insurance policy as a secondary insured" },
          { id: "ex7_vrtrust", label: "Onboard the property to VRTrust" },
          { id: "ex7_open_dates", label: "Final review and open future dates" },
        ],
        defaultFields: [],
      },
    ],
